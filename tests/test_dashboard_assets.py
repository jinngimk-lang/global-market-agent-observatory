...
def test_dashboard_assets_expose_required_observability_panels(tmp_path) -> None:
    ...
    with TestClient(app) as client:
        ...
        action_wiring = client.get("/static/action-wiring.js")
    ...
    assert action_wiring.status_code == 200
    ...
    assert 'src="/static/action-wiring.js"' in index.text
    assert index.text.index('/static/action-state.js') < index.text.index('/static/app.js')
    assert index.text.index('/static/app.js') < index.text.index('/static/action-wiring.js')
    ...
    assert "ObservatoryActionState.create" in action_wiring.text
    assert "removeEventListener('click', global.refreshAll)" in action_wiring.text
    assert "refreshAction.run(global.refreshAll)" in action_wiring.text
